...
window.Ashley = {
  responseCallback : function (responseObject){
    ...
    function undoEvent_set (e, t0 ,a ,t1){
      var eventType = e;
      var target = t0;
      var action = a;
      var task = t1;
      ...
      switch (eventType) {
        case 'addClass':
          console.log("Event = addClass, "+target+","+action+","+task);
          break;
        case 'addElenment':
          console.log("Event = addClass, "+target+","+action+","+task);
          break;
    ...
    function undoEvent_execute(){
      eval(undoString);
      undoString = "console.log('No undo event.')" ;
    ...
    function ALine() {
      ...
      $(".js_suggestions.regular").css({"display":"initial","left": pointsX[1]+10 + "px","top": pointsY[0] + shift -$(".js_suggestions").height()/2 + "px"});
    }
